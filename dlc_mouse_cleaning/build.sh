...
# Copy the built extension(s) next to this script so `import ceres_point_ba` works
# when running Python from the project root.
...
ls -1 "${ROOT_DIR}"/*.so 2>/dev/null || true
