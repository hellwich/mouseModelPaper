...
class PinholeCamera:
    ...
    R: np.ndarray  # 3x3
    t: np.ndarray  # 3
    dist: Optional[Sequence[float]] = None  # ignored (kept for compatibility)
...
def load_cameras_json(path: str) -> Dict[str, "PinholeCamera"]:
    """
    Loads the camera calibration JSON.

    Supports (at least) two formats:

    Format A (used by this project):
      {"cameras": [{"name": "...", "fx":..., "fy":..., "cx":..., "cy":..., "R": [[...]], "t": [...], "dist": [...]}, ...]}

    Format B (older draft):
      {"cameras": [{"name": "...", "intrinsics": {...}, "extrinsics": {...}}, ...]}

    The returned dict maps camera name -> PinholeCamera.
    """
    ...
    if isinstance(data, list):
        ...
        if "cameras" in data:
            ...
            # Some toolchains store a dict name->cam
            if all(isinstance(v, dict) for v in data.values()):
                cam_list = [dict({"name": k}, **v) for k, v in data.items()]

    if cam_list is None or not isinstance(cam_list, list):
    ...
    for idx, c in enumerate(cam_list):
        if not isinstance(c, dict):
            raise ValueError(f"Camera entry #{idx} must be an object/dict, got {type(c)}")
        ...
        if fx is None or fy is None or cx is None or cy is None:
            raise KeyError(f"Camera '{name}' is missing intrinsics (expected fx/fy/cx/cy or intrinsics/K).")

        # --- extrinsics ---
        R = t = None
        if "extrinsics" in c and isinstance(c["extrinsics"], dict):
            ext = c["extrinsics"]
            R = ext.get("R", ext.get("rotation", ext.get("R_wc", ext.get("R_cw", None))))
            t = ext.get("t", ext.get("translation", ext.get("t_wc", ext.get("t_cw", None))))
        else:
            R = c.get("R", c.get("rotation", None))
            t = c.get("t", c.get("translation", None))

        if R is None or t is None:
            raise KeyError(f"Camera '{name}' is missing extrinsics (expected R and t).")
