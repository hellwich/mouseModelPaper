...
def simulate(args: argparse.Namespace) -> None:
    ...
    path_ethogram = os.path.join(args.out_dir, "ethogram_gt.csv")
    ...
    feth = open(path_ethogram, "w", newline="", encoding="utf-8")
    weth = csv.writer(feth)
    weth.writerow(["frame", "time", "mouse0_behavior", "mouse1_behavior", "interaction", "mouse0_rear", "mouse1_rear", "mask_non_interaction"])
    ...
    for frame in range(n_frames):
        ...
        # write ground-truth ethogram (per-frame)
        b0, b1 = behavior_by_mouse[0], behavior_by_mouse[1]
        inter = 1 if (b0 == 2 or b1 == 2) else 0
        m0_rear = 1 if b0 == 1 else 0
        m1_rear = 1 if b1 == 1 else 0
        mask_non_inter = 0 if inter == 1 else 1
        weth.writerow([frame, f"{t:.6f}", b0, b1, inter, m0_rear, m1_rear, mask_non_inter])
    ...
    print(f"  Ground-truth ethogram: {path_ethogram}")
    for cam in cams:
